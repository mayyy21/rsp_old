...
#                label='Time Series - Channel {0:1d}'.format(chan)) # time data
        ...
        #        loud=i
        #        break
        ax.annotate(r'loud time: '+' = {0:5.5f}'.format(t_vec[loud]),
                     xy=(t_vec[loud],data[loud]),
                     xytext=(t_vec[loud],
                             data[loud]),
                     arrowprops=dict(facecolor='black',shrink=0.1)) # peak label
        loud_time = '{0:5.5f}'.format(t_vec[loud])
        #print('loud_time: '+'{0:5.5f}'.format(t_vec[loud]))
        #print('loud_time: ',loud_time)
        ...
        print('amplitude: ',data[loud])
        # peak finder labeling on the FFT plot
        max_indx = np.argmax(fft_data) # FFT peak index
        #print('fft_data.len: ',len(fft_data))
        #print('max_freq: ',max_indx)
        #print('freq_vec[max_indx]: ',freq_vec[max_indx])
        #print('fft_data[max_indx]: ',fft_data[max_indx])
...
if __name__=="__main__":
    ...
    #rate_tmp,data_tmp = wavfile.read(file_name)
    #t=np.arange(len(data_tmp[:,0]))*1.0/rate_tmp
    #pl.plot(t,data_tmp[:,0])
    #pl.show()



    #p=20*np.log10(np.abs(np.fft.rfft(data_tmp[:2048,0])))
    #f=np.linspace(0,rate_tmp/2.0,len(p))
    #pl.plot(f,p)
    #pl.xlabel('freq_hz')
    #pl.ylabel('power_db')
    #pl.show()
    ...
    for chan in range(0,chans):
        ...
        t=np.arange(len(data_tmp[:,0]))*1.0/rate_tmp
        pl.plot(t,data_tmp[:,0])
        print('data_tmp len:',len(data_tmp))
        #pl.show()
        ind = np.argmax(data_tmp)
        print('jjj: ',data_tmp[ind])


        arr[chan] = plotter(plt_1=1,plt_2=0) # select which data to plot
#        print('file_name: ',file_name)
        #rate_tmp,data_tmp = wavfile.read(file_name)
        #t=np.arange(len(data_tmp[:,0]))*1.0/rate_tmp
        #pl.plot(t,data_tmp[:,0])
...
#    print('loud_time1: ',arr[0])
#    print('loud_time2: ',arr[1])
#    delay = float(arr[0])-float(arr[1])
#    dif = delay
#    print('dif: ',dif)
#    tmp =0.2/(delay*349/65)
#    print('tmp: ',tmp)
    #theta = (180/math.pi)*np.arcsin(0.2/(delay*349))
#    theta=np.arcsin(tmp)
#    print('arcsin: ',np.arcsin(0.5))
#    print('theta: ',theta)
    plt.show() # show plot
    #print('loud_time1: ',arr[1])
    #print('loud_time2: ',arr[2])
